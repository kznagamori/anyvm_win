...
const String versionCacheJsonName = 'jdk_vm_version_cache.json';
const String vmName = 'JDKVm';
const String langName = 'JDKLang';
const String vmActivate = 'JDKVmActivate';
const String vmDeactivate = 'JDKVmDeactivate';

String getEnvDirectory() {
  String appDir = anyvm_util.getApplicationDirectory();
  String anyvmDir = Directory(appDir).parent.path;
  anyvm_util.logger.d(path.join(anyvmDir, 'envs', 'jdk'));
  return path.join(anyvmDir, 'envs', 'jdk');
}
...
List<String> getVersionDirectory() {
  ...
  for (var entity in entities) {
    if (entity is Directory) {
      RegExp pattern = RegExp(r'^\d+\.\d+\.\d+(_\d+)?');
...
Future<void> setVersion(String version) async {
  await unSetVersion();
  var jdkCurrentDirPath = path.join(getEnvDirectory(), 'current');
  anyvm_util.logger.d(jdkCurrentDirPath);
  var jdkCurrentDir = Directory(jdkCurrentDirPath);

  var jdkVersionDirPath = path.join(getEnvDirectory(), version);
  anyvm_util.logger.d(jdkVersionDirPath);
  var jdkVersionDir = Directory(jdkVersionDirPath);

  if (!await jdkVersionDir.exists()) {
    anyvm_util.logger.w('version does not exist');
    return;
  }
  if (await jdkCurrentDir.exists()) {
    var args = ['/C', 'RMDIR', jdkCurrentDirPath];
    ...
    if (result.exitCode != 0) {
      ...
      anyvm_util.logger.i('Derectory deleted: $jdkCurrentDirPath');
    }
  }
  var args = ['/C', 'MKLINK', '/J', jdkCurrentDirPath, jdkVersionDirPath];
  anyvm_util.logger.d('cmd.exe');
  for (var arg in args) {
    anyvm_util.logger.d(arg);
  }
  ...
  if (result.exitCode != 0) {
    ...
    anyvm_util.logger
        .i('Junction created: $jdkCurrentDirPath -> $jdkVersionDirPath');
  }

  var setPath = '${path.join(jdkCurrentDirPath, 'bin')};';
  ...
  scriptText += 'SET _OLD_JAVA_HOME=%JAVA_HOME%\n';
  scriptText += 'SET JAVA_HOME=$jdkCurrentDirPath\n';
  ...
  scriptText += '    \$env:_OLD_JAVA_HOME = \$env:JAVA_HOME;\n';
  scriptText += '    \$env:JAVA_HOME = "$jdkCurrentDirPath";\n';
  scriptText += '} else {\n';
  scriptText += '}\n';
  anyvm_util.logger.d(scriptText);
  ...
  scriptText += 'SET JAVA_HOME=%_OLD_JAVA_HOME%\n';
  scriptText += 'SET _OLD_JAVA_HOME=\n';
  scriptText += ':END_SET_ENV_VAL\n';
  anyvm_util.logger.d(scriptText);
  ...
  scriptText += '    \$env:JAVA_HOME = \$env:_OLD_JAVA_HOME;\n';
  scriptText += '    \$env:_OLD_JAVA_HOME = "";\n';
  scriptText += '}\n';
  anyvm_util.logger.d(scriptText);
...
Future<void> unSetVersion() async {
  ...
  var jdkVersionDirPath = path.join(getEnvDirectory(), currentVersion);
  anyvm_util.logger.d(jdkVersionDirPath);
  var jdkVersionDir = Directory(jdkVersionDirPath);
  ...
  if (await jdkCurrentDir.exists()) {
    ...
    anyvm_util.logger
        .i('Directory renamed/moved successfully.:$jdkCurrentDirPath');
  }
...
class DownloadItem {
  ...
  DownloadItem(this.version, this.browserDownloadUrl, this.fileName);

  @override
  String toString() {
    return 'Version: $version, URL: $browserDownloadUrl, File Name: $fileName';
  }
}
...
class JDKVm extends Command {
  @override
  final name = vmName;
  @override
  final description = 'JDK version manager.';

  JDKVm() {
    addSubcommand(JDKVmInstall());
    addSubcommand(JDKVmUpdate());
    addSubcommand(JDKVmVersions());
    addSubcommand(JDKVmVersion());
    addSubcommand(JDKVmSet());
    addSubcommand(JDKVmUnset());
    addSubcommand(JDKVmUnInstall());
  }
  @override
  void run() {
...
class JDKVmInstall extends Command {
  @override
  final name = 'install';
  @override
  ...
  Future<void> install(Map<String, dynamic> item) async {
    ...
    var jdkDirPath =
        path.join(getEnvCacheDirectory(), 'jdk-${item['version']}');

    var jdkDir = Directory(jdkDirPath);
    if (await jdkDir.exists()) {
      await jdkDir.rename(envVerDirPath);
      anyvm_util.logger.i('Directory renamed/moved successfully.: $jdkDirPath');
    }
    if (await file.exists()) {
      await file.delete();
...
class JDKVmUpdate extends Command {
  @override
  final name = 'update';
  @override
  ...
  Future<void> run() async {
    ...
    urls.add(
        'https://api.github.com/repos/adoptium/temurin11-binaries/releases');
    urls.add(
        'https://api.github.com/repos/adoptium/temurin17-binaries/releases');
    urls.add(
        'https://api.github.com/repos/adoptium/temurin21-binaries/releases');
    ...
    for (var url in urls) {
      ...
      if (response.statusCode == 200) {
        ...
        RegExp regex =
            RegExp(r'jdk_x64_windows_hotspot_(\d+\.\d+\.\d+(_\d+)?)\.zip$');

        for (var release in releases) {
          for (var asset in release['assets']) {
            String browserDownloadUrl = asset['browser_download_url'];
            String fileName = browserDownloadUrl.split('/').last;
            var match = regex.firstMatch(fileName);

            if (match != null) {
              String version = match.group(1)!;
              items.add(DownloadItem(version, browserDownloadUrl, fileName));
            }
          }
        }
      }
    ...
    for (var item in items) {
      Map<String, dynamic> versionMap = {
        'version': item.version.replaceAll('_', '+'),
...
class JDKVmVersions extends Command {
  @override
  final name = 'versions';
  @override
...
class JDKVmVersion extends Command {
  @override
  final name = 'version';
  @override
...
class JDKVmSet extends Command {
  @override
  final name = 'set';
  @override
...
class JDKVmUnset extends Command {
  @override
  final name = 'unset';
  @override
...
class JDKVmUnInstall extends Command {
  @override
