...
- **anyenv**には`global`、`local`の概念がありましたが、本ツールでは`set`/`unset`でバージョン切り替えのみとしています
- **Powershell（pwsh）**は、起動時に自動実行するスクリプトがあり、そこから本ツールのスクリプトを実行することで機能を有効化できます
- **コマンドプロンプト**はレジストリに起動時に自動実行するバッチファイルを設定できます。そのバッチファイルから本ツールのバッチファイルを実行することで機能を有効化できます
- 使い方によってWindows環境への影響度を調整できます
...
**Powershell（pwsh）**やコマンドプロンプトの起動時に自動実行するスクリプトで**anyvm_win**のスクリプトを実行します。**anyvm_win**で開発ツールの有効・無効を切り替えることで、Windows環境への影響度を設定します。
...
anyvm_winで有効化したツールは以下のディレクトリにジャンクションが張られます。このパスを環境変数のパスに追加することで、anyvm_winで有効化した開発ツールが使用できるようになります。各開発ツールへのジャンクションは以下となります。
...
`setup_jp.bat`を実行することで、起動時自動実行スクリプトの作成や設定が行われます。実行時には以下の質問がされます。
...
| -------- | --------------- | ------------ | ---- |
| install  | -l or --list    |              | インストール可能なバージョンの一覧を表示します（※1） |
| install  | -v or --version | バージョン   | 指定されたバージョンの開発ツールをインストールします<br>バージョンには※1で表示されるバージョンを指定する必要があります |
| install  | --latest        |              | 最新のバージョンの開発ツールをインストールします |
| uninstall | -v or --version | バージョン   | 指定したバージョンの開発ツールをアンインストールします |
...
| コマンド | 説明 |
| -------- | ---- |
| init     | インストールやanyvm自身の更新時に実行するコマンドです |
...
#### 4.4.4. バージョン3.9.13のPythonを有効にする
...
1. 以下のコマンドですべての開発ツールを無効にします。
```
anyvm unset
```

2. **anyvm_win**の展開ディレクトリを削除することでアンインストールできます。
...
| WinLibsVm    | PATH<br>LIBCLANG_PATH | envs\winlibs\current\bin;envs\winlibs\current\x86_64-w64-mingw32\bin<br>envs\winlibs\current\bin |
